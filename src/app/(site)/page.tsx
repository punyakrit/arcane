import TitleSection from "@/components/landingPage/titleSection";
import { Button } from "@/components/ui/button";
import React from "react";
import AppBanner from "../../../public/appBanner.png";
import Diamond from "../../../public/diamond.svg";
import Check from "../../../public/check.svg";

import Image from "next/image";
import { CLIENTS, PRICING_CARDS, PRICING_PLANS, USERS } from "@/lib/CONSTANT";
import { randomUUID } from "crypto";
import CustomCard from "@/components/landingPage/CustomCard";
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";
import { CardContent, CardDescription, CardTitle } from "@/components/ui/card";
function page() {
  return (
    <>
      <section className="overflow-hidden px-4 sm:px-6  sm:flex py-20  sm:flex-col gap-4 md:justify-center md:items-center">
        <TitleSection
          pill="✨ Your Ideas, Organized"
          title="The All-in-One Workspace for Teams and Creators"
        />
        <div className="p-[2px] bg-white mt-12 rounded-xl bg-gradient-to-r from-primary to-brand-primary-blue sm:w-[300px] cursor-pointer">
          <Button
            variant={"secondary"}
            className="w-full rounded-[13px] p-6 text-2xl bg-background cursor-pointer"
          >
            Get Arcane Free
          </Button>
        </div>
        <div className="md:mt-[-80px] sm:w-full w-[750px] flex justify-center items-center mt-[-40px] relative sm:ml-0 ml-[-50px]">
          <Image
            src={AppBanner}
            alt="App Banner"
            className="mask-b-from-35%"
          ></Image>
        </div>
      </section>
      <section className="relative">
        <div className="overflow-hidden flex after:content-[''] after:dark:from-brand-dark after:to-transparent after:from-background after:bg-gradient-to-l after:right-0 after:bottom-0 after:top-0 after:w-20 after:absolute after:z-10  before:content-[''] before:dark:from-brand-dark before:to-transparent before:from-background before:bg-gradient-to-r before:left-0 before:bottom-0 before:w-20 before:top-0  before:z-10 before:absolute">
          {[...Array(6)].map((_, index) => (
            <div key={index} className="flex flex-nowrap animate-slide">
              {CLIENTS.map((client, idx) => (
                <div
                  key={idx}
                  className="relative w-[200px] m-20 shrink-0 flex items-center"
                >
                  <Image
                    src={client.logo}
                    alt={client.alt}
                    width={200}
                    className=" object-contain max-w-none"
                  />
                </div>
              ))}
            </div>
          ))}
        </div>
      </section>
      <section className="px-4 sm:px-6 flex justify-center items-center flex-col relative mt-6">
        <div className="w-[30%] blur-[120px] rounded-full h-32 absolute bg-brand-primary-purple/50 -z-10 top-22 " />
        <TitleSection
          title="Everything you need to organize your work"
          subHeading="From notes to tasks, databases to whiteboards — bring all your tools into one connected workspace."
          pill="Features"
        />
      </section>

      <section className="relative overflow-hidden">
        <div className="w-full blur-[120px] rounded-full h-32 absolute bg-brand-primary-purple/50 -z-10 top-56 " />
        <TitleSection
          title="Loved by teams and individuals worldwide"
          subHeading="From startups to global enterprises, creators to educators — thousands of people trust our platform to power their daily productivity."
          pill="Testimonials"
        />
        <div>
          {[...Array(2)].map((arr, index) => (
            <div
              key={randomUUID()}
              className={`mt-10 flex flex-nowrap gap-6 self-start
    ${index === 1 ? "flex-row-reverse" : ""}
    ${true ? "animate-[slide_250s_linear_infinite]" : ""}
    ${index === 1 ? "animate-[slide_250s_linear_infinite_reverse]" : ""}
    ${index === 1 ? "ml-[100vw]" : ""}
    hover:paused
  `}
            >
              {USERS.map((user, idx) => (
                <CustomCard
                  key={idx}
                  className="w-[500px] shrink-0 rounded-xl
                dark:bg-gradient-to-t dark:from-border dark:to-background
                "
                  CustomHeader={
                    <div className="flex items-center gap-4">
                      <Avatar>
                        <AvatarImage
                          src={`/avatars/${idx + 1}.png`}
                          alt={user.name}
                        />
                        <AvatarFallback>AV</AvatarFallback>
                      </Avatar>
                      <div>
                        <CardTitle className="text-foreground">
                          {user.name}
                        </CardTitle>
                        <CardDescription className="dark:text-washed-purple-800">
                          {user.name.toLowerCase()}
                        </CardDescription>
                      </div>
                    </div>
                  }
                  CustomContent={
                    <p className="dark:text-washed-purple-800">
                      {user.message}
                    </p>
                  }
                ></CustomCard>
              ))}
            </div>
          ))}
        </div>
      </section>

      <section className="mt-20 px-4 sm:px-6">
        <TitleSection
          title="Simple, Transparent Pricing"
          subHeading="Choose a plan that fits your needs and budget. No hidden fees, no surprises—just great value."
          pill="Pricing"
        />
        <div className="flex flex-col-reverse sm:flex-row gap-4 justify-center sm:items-stretch items-center mt-10">
          {PRICING_CARDS.map((card, index) => (
            <CustomCard
              key={card.planType}
              className={`w-[300px] rounded-2xl dark:bg-black/40 backdrop-blur-3xl relative  ${
                card.planType === PRICING_PLANS.proplan &&
                "border-brand-primary-purple/70"
              }`}
              CustomHeader={
                <CardTitle className="text-3xl font-semibold">
                  {card.planType === PRICING_PLANS.proplan && (
                    <>
                      <div className="hidden dark:block  w-full blur-[120px] rounded-full h-32 absolute bg-brand-primary-purple/80 -z-10 top-0"></div>
                      <Image
                        src={Diamond}
                        alt="Pro Plan Icon"
                        width={50}
                        height={50}
                        className="absolute top-6 right-6"
                      ></Image>
                    </>
                  )}
                  {card.planType}
                </CardTitle>
              }
              CustomContent={
                <CardContent className="p-0">
                  <span className="font-normal text-2xl">${card.price}</span>
                  {+card.price > 0 ? (
                    <span className="dark:text-washed-purple-800 ml-1">
                      /mo
                    </span>
                  ) : (
                    ""
                  )}
                  <p className="text-washed-purple-800">{card.description}</p>
                  <Button className="whitespace-nowrap mt-4 w-full">
                    {card.planType === PRICING_PLANS.proplan
                      ? "Upgrade to Pro"
                      : "Get Started"}

                  </Button>
                </CardContent>
              }
              CustomFooter={
                <ul className="font-normal flex mb-2 flex-col gap-4">
                  <small>{card.highlightFeature}</small>
                  {card.freatures.map((feature, idx) => (
                    <>
                      <li key={idx} className="flex items-center gap-2">
                        <Image src={Check} alt={feature}></Image>
                        {feature}
                      </li>
                    </>
                  ))}
                </ul>
              }
            />
          ))}
        </div>
      </section>
    </>
  );
}

export default page;
